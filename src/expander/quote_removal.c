...
static int	quoted_len(char *word, t_word *word_lst)
{
	...
	while (*word)
	{
		if (*word != in_qt)
			len++;
		if (*word == in_qt && in && !word_lst->literal)
		{
			in = false;
			if (word_lst->in_quote && *(ft_strchr(word, 0) - 1) == word_lst->in_quote) 
				in_qt = word_lst->in_quote;
			else
...
static	void	add_no_quotes(char **str, char **new, t_word *word)
{
	...
	while (**str)
	{
		if (**str != in_qt)
			new[0][++i] = *(*str);
		if (**str == in_qt && in && !word->literal)
		{
			in = false;
			if (word->in_quote && *(ft_strchr(*str, 0) - 1) == word->in_quote) 
				in_qt = word->in_quote;
			else
				in_qt = find_in(*(str) + 1);
		}
