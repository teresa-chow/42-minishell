...
/*   Updated: 2025/04/23 11:44:49 by tchow-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	while (i)
	{
		...
		if (root->word)
		{
			int	old_stdin = dup(STDIN_FILENO); //norm
			int	old_stdout = dup(STDOUT_FILENO); //norm
			ast_depth_search(&data, &root, &i);
			reset_old_in_out(old_stdin, old_stdout); //norm
		}
